...
onready var default_position := Vector2(640.0, 100.0)
onready var offset_allowed := Vector2(300.0, 20.0)
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_left") and position.x > limits.left:
		position.x -= moveable_value
	elif event.is_action_pressed("ui_right") and position.x < limits.right:
		position.x += moveable_value
